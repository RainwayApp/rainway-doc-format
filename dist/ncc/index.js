...
const _1 = __nccwpck_require__(283);
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const input = {
                ...
                mode: core.getInput("xmldoc") === "true" ? _1.Mode.Xmldoc : _1.Mode.ApiExtractor,
...
run();
//# sourceMappingURL=action-rainway-doc-format.js.map
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.format = exports.handleApiExtractorLink = exports.handleXmldocLink = exports.handleAnyLink = exports.Mode = void 0;
const path = __importStar(__nccwpck_require__(17));
const fs_1 = __nccwpck_require__(147);
const glob_1 = __nccwpck_require__(90);
const util_1 = __nccwpck_require__(251);
...
(function (Mode) {
     ...
     */
    Mode[Mode["ApiExtractor"] = 0] = "ApiExtractor";
    /**
     * Xmldoc formatter
     */
    Mode[Mode["Xmldoc"] = 1] = "Xmldoc";
 ...
 * @param props - handler props
 */
function handleXmldocLink(link, props) {
    ...
    const res = handleAnyLink(Object.assign(Object.assign({}, link), { href: hierarchy }), props);
...
exports.handleXmldocLink = handleXmldocLink;
/**
 * Handler for api extractor links
 * @param link - md link
 * @param props - handler props
 */
function handleApiExtractorLink(link, props) {
    const res = handleAnyLink(Object.assign({}, link), props);
...
exports.handleApiExtractorLink = handleApiExtractorLink;
/**
...
function format(input) {
    return __awaiter(this, void 0, void 0, function* () {
        const globber = yield (0, glob_1.create)(input.input);
        ...
        yield Promise.all(files.map((file) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (extName !== ".md") {
                console.warn(`Warning: File '${file}' does not have the .md extension. Formatting may yield undesirable results.`);
            }
            const data = yield fs_1.promises.readFile(file, { encoding: "utf-8" });
            const output = (0, util_1.forAllLinks)(data, (link) => {
            ...
            if (baseName.includes(input.srcLib)) {
                baseName = baseName.replace(new RegExp(input.srcLib, "g"), input.dstLib);
            }
            const outFile = (0, util_1.escapeFilename)((path.isAbsolute(input.output)
                ? path.join(input.output, baseName)
                : path.join(dirName, input.output, baseName)) + extName);
            yield fs_1.promises.mkdir(path.dirname(outFile), { recursive: true });
            if (outFile !== file) {
                console.log(`Renaming file '${file}' => '${outFile}'.`);
                yield fs_1.promises.rename(file, outFile);
            }
            yield fs_1.promises.writeFile(outFile, output, { encoding: "utf-8" });
        })));
        return 0;
    });
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.forAllLinks = exports.escapeFilename = exports.findPossibleLibraryName = void 0;
const path = __importStar(__nccwpck_require__(17));
/**
 ...
 * @returns string
 */
function findPossibleLibraryName(input) {
...
exports.findPossibleLibraryName = findPossibleLibraryName;
/**
 * Docs-safe file path
...
function escapeFilename(input) {
    ...
    const output = `${path.join(dirName, baseName)}${extName}`.replace(/\\/g, "/");
...
function forAllLinks(input, cb) {
    ...
    while ((match = mdLinkRe.exec(input)) !== null) {
        ...
        if (repl !== null) {
            replacements.push(Object.assign(Object.assign({}, repl), { start: match.index, end: match.index + len }));
        }
    }
    for (let i = 0; i < replacements.length; i++) {
        const re = replacements[i];
        const beforeSize = input.length;
        input = `${input.substring(0, re.start)}[${re.title}](${re.href})${input.substring(re.end)}`;
...
exports.forAllLinks = forAllLinks;
